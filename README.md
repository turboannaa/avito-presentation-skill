...
## Контроль количества слайдов

Поле `expected_slide_count` в plan.json — опциональное. Правило:

- **Документ явно нумерует разделы** (`## 1.`, `Слайд 1`, `Section 3` и т.п.) → Клод ставит `expected_slide_count` равным числу разделов. Скрипт проверяет совпадение и падает с ошибкой до запуска, если слайдов больше или меньше.
- **Документ без явной нумерации** → поле не ставится, количество слайдов выбирается по смыслу.
